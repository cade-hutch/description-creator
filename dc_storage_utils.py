...
def upload_images_from_dir(folder_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith((".png")):
            t_start = time.perf_counter()
            blob = bucket.blob(os.path.join('dc_images', folder_name, filename))
            t_end1 = time.perf_counter()
            print('finished db connection in {}s'.format(round(t_end1 - t_start, 2)))
            ...
            print('finished {} upload in {}s'.format(os.path.join(folder_path, filename), round(t_end - t_start, 2)))
...
def upload_json_descriptions_file(json_descriptions_file):
    ...
    if json_descriptions_file.endswith((".json")):
        blob = bucket.blob(os.path.join('dc_json', json_descriptions_filename))
...
def does_image_folder_exist(folder_name):
    images_dir = "dc_images/{}".format(folder_name)
    bucket = storage.bucket('image-finder-demo.appspot.com')
...
def download_images(remote_folder, local_folder):
    if not remote_folder.startswith('dc_images/'):
        remote_folder = os.path.join('dc_images', remote_folder)
    print('a')
    bucket = storage.bucket('image-finder-demo.appspot.com')
    ...
    for blob in blobs:
        ...
        if blob.name.lower().endswith('.png'):
            ...
            if not os.path.exists(file_path):
                ...
                print(f"image already exists localy: {file_path}")
